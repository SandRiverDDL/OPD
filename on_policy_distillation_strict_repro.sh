...
# Resolve the shared Hugging Face cache used by this training instance. The
# training shell may not inherit HF_HOME from the terminal that downloaded the
# models, so prefer the instance's persistent cache when it is present.
if [ -z "${HF_HOME:-}" ]; then
    HF_CANDIDATE_DIRS=()
    if [ -n "${TAIJI_IDE_DEV_DIR:-}" ]; then
        HF_CANDIDATE_DIRS+=("${TAIJI_IDE_DEV_DIR%/dev}/.cache/huggingface")
    fi
    HF_CANDIDATE_DIRS+=("${SCRIPT_DIR%/dev/lab/OPD}/.cache/huggingface")
    for hf_candidate in "${HF_CANDIDATE_DIRS[@]}"; do
        if [ -d "$hf_candidate" ]; then
...
hf_repo_snapshot_path() {
    local repo_id="$1"
    local repo_kind="${2:-models}"
    local repo_dir="$HF_HUB_CACHE/${repo_kind}--${repo_id//\//--}"
    local revision
    if [ ! -d "$repo_dir" ] || [ ! -f "$repo_dir/refs/main" ]; then
        return 1
    fi
...
HF_STUDENT_MODEL_PATH="${HF_STUDENT_MODEL_PATH:-$(hf_repo_snapshot_path deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B models || true)}"
HF_TEACHER_MODEL_PATH="${HF_TEACHER_MODEL_PATH:-$(hf_repo_snapshot_path hbx/JustRL-DeepSeek-1.5B models || true)}"
HF_DAPO_SNAPSHOT_PATH="${HF_DAPO_SNAPSHOT_PATH:-$(hf_repo_snapshot_path BytedTsinghua-SIA/DAPO-Math-17k datasets || true)}"
HF_DAPO_DATASET_PATH="${HF_DAPO_DATASET_PATH:-${HF_DAPO_SNAPSHOT_PATH:+$HF_DAPO_SNAPSHOT_PATH/data/dapo-math-17k.parquet}}"
...
export N_RESPONSES=${N_RESPONSES:-4} # Paper default: four student rollouts per prompt
...
export MODEL_DTYPE=${MODEL_DTYPE:-fp32} # Match the original launcher default
...
export ACTOR_MODEL_PATH=${ACTOR_MODEL_PATH:-${HF_STUDENT_MODEL_PATH:-model/DeepSeek-R1-Distill-Qwen-1.5B}}
...
# export ACTOR_MODEL_PATH=model/DS-1.5B-SFT
export ACTOR_MODEL_NAME=${ACTOR_MODEL_NAME:-DeepSeek-R1-Distill-Qwen-1.5B}
...
export REWARD_MODEL_PATH=${REWARD_MODEL_PATH:-${HF_TEACHER_MODEL_PATH:-model/JustRL-DeepSeek-1.5B}}
export REWARD_MODEL_NAME=${REWARD_MODEL_NAME:-JustRL-DeepSeek-1.5B}
...
if [[ "$TRAIN_DATASET" != /* ]]; then
    TRAIN_DATASET="$SCRIPT_DIR/$TRAIN_DATASET"
fi
if [[ "$TEST_DATA_DIR" != /* ]]; then
    TEST_DATA_DIR="$SCRIPT_DIR/$TEST_DATA_DIR"
fi
if [[ "$ACTOR_MODEL_PATH" != /* ]]; then
    ACTOR_MODEL_PATH="$SCRIPT_DIR/$ACTOR_MODEL_PATH"
fi
if [[ "$REWARD_MODEL_PATH" != /* ]]; then
    REWARD_MODEL_PATH="$SCRIPT_DIR/$REWARD_MODEL_PATH"
fi
...
for required_path in "$TRAIN_DATASET" "$ACTOR_MODEL_PATH" "$REWARD_MODEL_PATH"; do
    if [ ! -e "$required_path" ]; then
        echo "ERROR: required path does not exist: $required_path" >&2
        exit 2
    fi
done
for required_test in "$TEST_DATA_DIR/MATH-500/test.parquet"; do
    if [ ! -e "$required_test" ]; then
        echo "ERROR: required validation file does not exist: $required_test" >&2
        exit 2
    fi
done

export TRAIN_DATASET TEST_DATA_DIR ACTOR_MODEL_PATH REWARD_MODEL_PATH
export PYTHONPATH="$SCRIPT_DIR/verl${PYTHONPATH:+:$PYTHONPATH}"

echo "Resolved OPD paths:"
...
if [ "${DRY_RUN:-False}" = "True" ]; then
    echo "DRY_RUN=True; stopping before Ray startup."
    exit 0
fi
...
if [ -z "${PPO_MAX_TOKEN_LEN_PER_GPU:-}" ]; then
    # This budget applies only to the actor PPO forward/backward passes.
    # Keep it separate from log-prob and vLLM generation budgets:
    # 64K here was observed to OOM during actor backward on 95 GiB GPUs.
...
export REF_LOG_PROB_MAX_TOKEN_LEN_PER_GPU=${REF_LOG_PROB_MAX_TOKEN_LEN_PER_GPU:-16384}
export TEACHER_MAX_TOKEN_LEN_PER_GPU=${TEACHER_MAX_TOKEN_LEN_PER_GPU:-16384}
echo "PPO_MAX_TOKEN_LEN_PER_GPU (actor): $PPO_MAX_TOKEN_LEN_PER_GPU"
echo "ROLLOUT_LOG_PROB_MAX_TOKEN_LEN_PER_GPU (actor log-prob): $ROLLOUT_LOG_PROB_MAX_TOKEN_LEN_PER_GPU"
echo "REF_LOG_PROB_MAX_TOKEN_LEN_PER_GPU (reference log-prob): $REF_LOG_PROB_MAX_TOKEN_LEN_PER_GPU"
echo "TEACHER_MAX_TOKEN_LEN_PER_GPU (teacher/reward): $TEACHER_MAX_TOKEN_LEN_PER_GPU"
echo "VLLM_MAX_NUM_BATCHED_TOKENS (rollout): $VLLM_MAX_NUM_BATCHED_TOKENS"
echo "VLLM_MAX_NUM_SEQS (rollout): $VLLM_MAX_NUM_SEQS"
echo "VLLM_GPU_MEMORY_UTILIZATION (rollout): $VLLM_GPU_MEMORY_UTILIZATION"
